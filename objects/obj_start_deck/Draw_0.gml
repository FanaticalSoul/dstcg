...
//var _sprites = obj_player.sprites;
	...
	if (deck[i][0]!="") { // existing cards
		if (deck_reveal) {
			...
			if (deck[i][1] || deck_reveal_full) {
				//draw_sprite(_sprites[deck[i][0]],-1,x+i*deck_spacing,y-i*deck_spacing);
				var _sprite = spr_card_back;
				for (j = 0; j < array_length(start_card_stats); j++) {
